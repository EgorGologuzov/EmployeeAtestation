...
{
    public class ViewList<TModel, TView> : ObservableCollection<TView> where TView : ViewModelBase<TModel>
    {
        ...
        public ViewList(IList<TModel> models)
        {
        ...
        private void RemoveViews(IList models)
        {
            ...
            foreach (TModel model in models)
            {
                TView view = this.Where(v => v.Model.Equals(model)).First();
                this.Remove(view);
            }
